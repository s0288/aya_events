...
def test_event_listener__get_update_and_next_update_id(json, expected):
    assert EventListener._get_update_and_next_update_id(json) == expected
...
@pytest.mark.parametrize("update_message, expected", [
    (
        ...
        , ('TELEGRAM_USER_ID', 'hej', 813, 'TELEGRAM_USER_ID')
    ), (
        ...
        , ('TELEGRAM_USER_ID', 'file: FILE_ID_0', 850, 'TELEGRAM_USER_ID')
    ), (
        ...
        , ('TELEGRAM_USER_ID', 'file: FILE_ID_0', 856, 'TELEGRAM_USER_ID')
    ), (
        ...
        , ('TELEGRAM_USER_ID', 'file: FILE_ID_0', 857, 'TELEGRAM_USER_ID')
    )
    ], ids=["regular", "img upload", "doc upload 1", "doc upload 2"]
)
def test_event_listener__extract_message(update_message, expected):
    assert EventListener._extract_message(update_message) == expected


@pytest.mark.parametrize("update_message, expected", [
    (
        {"update_id":18282878,
            "callback_query":{'id': '1785005942255623485', 'from': {'id': 'TELEGRAM_USER_ID', 'is_bot': False, 'first_name': 'TELEGRAM_FIRST_NAME', 'language_code': 'en'}, 
        ...
        , ('TELEGRAM_USER_ID', 'ja', 805, 'TELEGRAM_USER_ID', 'Du fastest seit 12 Stunden und 17 Minuten. Möchtest du das Fasten jetzt beenden?')
    )
    ], ids=["regular"]
)
def test_event_listener__extract_callback(update_message, expected):
    print(EventListener._extract_callback(update_message))
    assert EventListener._extract_callback(update_message) == expected
